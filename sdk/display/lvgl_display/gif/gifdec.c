...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    if(memcmp(sigver, "GIF", 3) != 0) {
        LINX_LOGW(TAG, "invalid signature");
        goto fail;
    }
    /* Version */
    f_gif_read(gif_base, sigver, 3);
    if(memcmp(sigver, "89a", 3) != 0 && memcmp(sigver, "87a", 3) != 0) {
        LINX_LOGW(TAG, "invalid version");
        goto fail;
    }
    /* Width x Height */
    ...
    /* Presence of GCT */
    if(!(fdsz & 0x80)) {
        LINX_LOGW(TAG, "no global color table");
        goto fail;
    }
    /* Color Space's Depth */
    depth = ((fdsz >> 4) & 7) + 1;
    ...
    if(0 == width || 0 == height){
        LINX_LOGW(TAG, "Zero size image");
        goto fail;
    }
#if LV_GIF_CACHE_DECODE_DATA
    if(0 == (INT_MAX - sizeof(gd_GIF) - LZW_CACHE_SIZE) / width / height / 5){
        LINX_LOGW(TAG, "Image dimensions are too large");
        goto fail;
    } 
    ...
    gif->lzw_cache_size = LZW_CACHE_SIZE;
    #endif
...
read_ext(gd_GIF * gif)
{
    ...
    switch(label) {
        ...
        case 0xFF:
            ...
            break;
        default:
            LINX_LOGW(TAG, "unknown extension: %02X", label);
    }
}
...
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    while (frm_off < frm_size) {
        ...
        while (sp > p_stack) {
            if(frm_off >= frm_size){
                LINX_LOGW(TAG, "LZW table token overflows the frame buffer");
                return -1;
            }
            *ptr++ = *(--sp);
            frm_off += 1;
...
read_image(gd_GIF * gif)
{
    ...
    if(gif->fx + (uint32_t)gif->fw > gif->width || gif->fy + (uint32_t)gif->fh > gif->height){
        LINX_LOGW(TAG, "Frame coordinates out of image bounds");
        return -1;
    }
